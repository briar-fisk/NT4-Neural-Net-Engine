		...
		for (int cou_T = 0; cou_T < State_Depth; cou_T++)
		{
			for (int cou_Input = 0; cou_Input < (State_Depth - cou_T); cou_Input++)
			{
				if (Scaffold[cou_T][cou_Input] != NULL)
				{
					...
					if (p_Style == -1)
					{
						tmp_Buffman.submit(Scaffold[cou_T][cou_Input], (10.0));
					}
					if (p_Style == 1)
					{
						tmp_Buffman.charge_Given_Leg(Scaffold[cou_T][cou_Input], (10.0), cou_Input);
					}
					if (p_Style == 2)
					{
						//p_Leg specifies which leg to charge in this function, p_Legs[] being unused.
						tmp_Buffman.charge_Given_Leg(Scaffold[cou_T][cou_Input], (10.0), p_Leg);
					}
					if (p_Style == 3)
					{
						//p_Leg is used here as the count of elements in p_Legs[].
						tmp_Buffman.charge_Given_Legs(Scaffold[cou_T][cou_Input], p_Leg, p_Legs, (10.0));
					}
				}
			}
		}
